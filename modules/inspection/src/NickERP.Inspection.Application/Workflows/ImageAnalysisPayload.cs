...
/// <c>inspection.queue_image_analysis</c>. Captures the minimal reference
/// set the (vendor-neutral) image-analysis consumer needs to dispatch
/// inference work for a case once split-detection has settled. Real
/// fields land alongside the consumer's actual implementation in a later
...
/// image-analysis work was enqueued for. Used for log correlation in the
...
/// scaffold end-to-end as compile-clean stubs; the actual image-analysis
/// dispatch (which routes inference per the active threshold profile +
/// scanner type) lands in a later sprint. The payload is deliberately
/// the smallest set of fields that will keep working when the real
/// consumer body shows up — adding fields later is a forward-compatible
/// JSON change; renaming or removing them is not.
/// </para>
/// </remarks>
public sealed record ImageAnalysisPayload(Guid WorkItemId, Guid CaseId, DateTimeOffset EnqueuedAt);
