...
/// <c>inspection.queue_decision_agent</c>. Captures the minimal
/// reference set the (rules-based) decision-agent consumer needs to
/// run its scoring + decision pass against a case after image-analysis
/// has produced findings. Real fields land alongside the consumer's
/// actual implementation in a later sprint; the placeholders below keep
/// the wire format tractable for tests + logs.
/// </summary>
...
/// decision-agent work was enqueued for. Used for log correlation in
...
/// scaffold end-to-end as compile-clean stubs; the actual
/// decision-agent dispatch (shadow vs. live posture, condition-weight
/// resolution, audit-trail emission) lands in a later sprint. The
...
/// </para>
/// </remarks>
public sealed record DecisionAgentPayload(Guid WorkItemId, Guid CaseId, DateTimeOffset EnqueuedAt);
