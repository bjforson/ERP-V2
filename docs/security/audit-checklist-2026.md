...
**Resolved finding (Sprint 58, 2026-05-06):** the Sprint 57 P0 — **NBomber 6.1.0** shipping under a proprietary commercial subscription — is **RESOLVED** by removing NBomber + NBomber.Http + NBomber.Contracts from `tests/NickERP.Perf.Tests/NickERP.Perf.Tests.csproj` and replacing the runtime with a homegrown in-tree runner under `tests/NickERP.Perf.Tests/Runner/` (`NickPerfScenario`, `NickPerfRunner`, `NickPerfStats`, `NickPerfReport`, `Http/NickPerfHttp`). Behaviour parity preserved: same per-profile RPS, same skip-on-misconfigured semantics, same acceptance-gate thresholds. Post-Sprint-58 audit re-run (`tools/security-scan/reports/2026-05-06-license-audit.md`) shows **0 non-allowlisted licenses, 1 unresolved (xunit.abstractions, already triaged via override)**. SEC-DEP-3 has no open P0 findings. Full audit trail in `tools/security-scan/license-allowlist-rationale.md` §2.
