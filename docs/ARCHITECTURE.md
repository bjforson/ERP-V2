...
**Race-eviction note.** `SourceJanitorWorker.SweepTenantAsync` reduces candidates by content hash and only evicts when no row in the *current tenant's* RLS-narrowed view still references the hash from a non-Closed/Cancelled case (see `modules/inspection/src/NickERP.Inspection.Imaging/SourceJanitorWorker.cs`, the per-tenant `SweepTenantAsync` method — projects candidates whose case is `Closed`/`Cancelled` past `SourceRetentionDays`, then loads the `stillReferenced` hash set from non-evictable cases and skips eviction for any hash in that set). The check is correct *within* a tenant; it is not cross-tenant. A genuine cross-tenant guard would require either:
