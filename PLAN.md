...
- **Sprint 57 license audit triage surfaced NBomber as a P0 finding.** NBomber 6.1.0 ships under proprietary commercial subscription terms, NOT MIT. csproj comment was incorrect. Resolution path is operator-decision (purchase / replace / drop perf harness). Documented under SEC-DEP-3 in `docs/security/audit-checklist-2026.md` + `tools/security-scan/license-allowlist-rationale.md` §2. **Resolved Sprint 58 (2026-05-06):** NBomber + NBomber.Http + NBomber.Contracts removed; replaced by the homegrown in-tree NickPerf runner at `tests/NickERP.Perf.Tests/Runner/`. SEC-DEP-3 P0 closed; see audit-checklist `SEC-DEP-3` for full trail.
...
7. Execute Phase V proper — `docs/security/audit-checklist-2026.md` (~89 SEC-* items) + `docs/perf/test-plan.md`. Resolve all P0 + P1 findings before launch. SEC-DEP-3 NBomber license was the last open P0; closed Sprint 58 (NickPerf in-tree runner replaces NBomber). Runner: `tools/security-scan/run-audit.ps1`; perf wrapper: `tools/perf/run-phase-v.ps1` (requires pwsh 7+, per `fix(tools): PowerShell encoding + version gating` 2026-05-14).
